...
func _process(delta: float) -> void:
	...
	if _fox_to_hare_timer <= 0.0:
		...
		var hare_bonus_gain: int = get_hare_per_fox_gain_count()
		if hare_bonus_gain > 0:
			SaveManager.add_dna(SaveManager.DNAType.Hare, hare_bonus_gain)
...
func get_fox_to_hares_level() -> int:
	return int(SaveManager.get_value(_get_key(FOX_TO_HARE_LEVEL_SUFFIX), 0))
